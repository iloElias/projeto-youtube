/* Opções pessoais */
let optionsArray = [
    // [nome da opção, svg path]
    ["Biblioteca", "m11 7 6 3.5-6 3.5V7zm7 13H4V6H3v15h15v-1zm3-2H6V3h15v15zM7 17h13V4H7v13z"],
    ...
    ["Seus vídeos", "m10 8 6 4-6 4V8zm11-5v18H3V3h18zm-1 1H4v16h16V4z"],
    ["Assistir mais tarde", "M14.97 16.95 10 13.87V7h2v5.76l4.03 2.49-1.06 1.7zM12 3c-4.96 0-9 4.04-9 9s4.04 9 9 9 9-4.04 9-9-4.04-9-9-9m0-1c5.52 0 10 4.48 10 10s-4.48 10-10 10S2 17.52 2 12 6.48 2 12 2z"],
    ["Vídeos marcados com gostei", "M18.77,11h-4.23l1.52-4.94C16.38,5.03,15.54,4,14.38,4c-0.58,0-1.14,0.24-1.52,0.65L7,11H3v10h4h1h9.43 c1.06,0,1.98-0.67,2.19-1.61l1.34-6C21.23,12.15,20.18,11,18.77,11z M7,20H4v-8h3V20z M19.98,13.17l-1.34,6 C18.54,19.65,18.03,20,17.43,20H8v-8.61l5.6-6.06C13.79,5.12,14.08,5,14.38,5c0.26,0,0.5,0.11,0.63,0.3 c0.07,0.1,0.15,0.26,0.09,0.47l-1.52,4.94L13.18,12h1.35h4.23c0.41,0,0.8,0.17,1.03,0.46C19.92,12.61,20.05,12.86,19.98,13.17z"],
    ["Mostrar mais", "m18 9.28-6.35 6.35-6.37-6.35.72-.71 5.64 5.65 5.65-5.65z"]
]
...
let htmlTemplate = `
    <div class="guide-option">
        ...
        <div class="guide-text">OPTIONNAME</div>
    </div>
`
let htmlMostrarMais = htmlTemplate.replace("OPTIONNAME", optionsArray[optionsArray.length - 1][0])
htmlMostrarMais = htmlMostrarMais.replace("SVGPATHD", optionsArray[optionsArray.length - 1][1])

let optionsContainer = document.getElementById("personal-options")

for (let i in optionsArray) {
    let newHtml = htmlTemplate.replace("OPTIONNAME", optionsArray[i][0])
    newHtml = newHtml.replace("SVGPATHD", optionsArray[i][1])
...
/* Opções de canais inscritos */
let inscricoesArray = [
    // [nome do canal, imagem do canal, isInLive],
    ["Diolinux", "https://yt3.ggpht.com/aypXJDLRC-q7U8nNFRIM07EozoAe1neU2F_hNsbMtuFp6XLHFscxhK73JG-mVbjz_YuABStM-60=s88-c-k-c0x00ffffff-no-rj", true],
    ["Alan Becker", "https://yt3.ggpht.com/ytc/AOPolaTjhMx__R3JHuQHwsLYhRV76ipKhWUfNf3WXDbNJg=s88-c-k-c0x00ffffff-no-rj", true],
    ["Lofi Girl", "https://yt3.ggpht.com/gY8H7K-3Eg3olVftRBiqqFe-N5d9Rx90jAsrfQuxDa4m32Wm-kWK6AQJhwchvYLf-H4EjGhCSw=s88-c-k-c0x00ffffff-no-rj", true],
    ["GameChops", "https://yt3.ggpht.com/ytc/AOPolaRevjDau4yEDiQSjaGN031cJZZD_WSSo-d3-CAhUw=s88-c-k-c0x00ffffff-no-rj", true],
    ["Felca", "https://yt3.ggpht.com/ytc/AOPolaQDm2w-8hiGkmEqeIAykBD0sI2rX5cfsdlWM6QqhQ=s88-c-k-c0x00ffffff-no-rj", false],
    ["Saiko joga", "https://yt3.ggpht.com/ytc/AOPolaSCntjMBvvHTZMAN4ID-KvlnKwK0OFXbLIn34_8wg=s88-c-k-c0x00ffffff-no-rj", false],
    ["Davi", "https://yt3.ggpht.com/K8C4Ahv9fQiH2D4Rn8DFgstf7T2FO4LsMEgifYliFHNWRnM7ogfNSc-KoPJmp0hth5TZfKHdvA=s88-c-k-c0x00ffffff-no-rj", false],
]

htmlTemplate = `
    <div class="guide-option">
        <img height="24" width="24" alt="" src="PROFILEIMAGE">
        <div class="guide-text">CHANELNAME</div>
        <div style="display: NONEORFLEX; fill: #f00;">
...
`
let addContentContainer = document.getElementById("Inscricoes")

for (let i in inscricoesArray) {
    let newHtml = htmlTemplate.replace("CHANELNAME", inscricoesArray[i][0])
    newHtml = newHtml.replace("PROFILEIMAGE", inscricoesArray[i][1])

    if (inscricoesArray[i][2]) {
        newHtml = newHtml.replace("NONEORFLEX", "flex")
    } else {
        newHtml = newHtml.replace("NONEORFLEX", "none")
    }

    addContentContainer.innerHTML += newHtml
}

addContentContainer.innerHTML += htmlMostrarMais

let exploreArray = [
    // [nome da opção, svg path]
    ["Em alta", "M19 3.87v9.77C19 17.7 15.86 21 12 21s-7-3.3-7-7.37v-.13c0-1.06.22-2.13.62-3.09.5-1.19 1.29-2.21 2.27-2.97.85-.66 1.83-1.14 2.87-1.65.39-.19.77-.38 1.15-.58.36-.19.72-.38 1.08-.56v3.22l1.55-1.04L19 3.87M20 2l-6 4V3c-.85.44-1.7.88-2.55 1.33-1.41.74-2.9 1.34-4.17 2.32-1.13.87-2.02 2.05-2.58 3.37-.46 1.09-.7 2.29-.7 3.48v.14C4 18.26 7.58 22 12 22s8-3.74 8-8.36V2zM9.45 12.89 14 10v5.7c0 1.82-1.34 3.3-3 3.3s-3-1.47-3-3.3c0-1.19.58-2.23 1.45-2.81z"],
    ...
    ["Filme", "m22.01 4.91-.5-2.96L1.64 5.19 2 8v13h20V8H3.06l18.95-3.09zM5 9l1 3h3L8 9h2l1 3h3l-1-3h2l1 3h3l-1-3h3v11H3V9h2z"],
...
let exploreContainer = document.getElementById("Explorar")

for (let i in exploreArray) {
    let newHtml = htmlTemplate.replace("OPTIONNAME", exploreArray[i][0])
    newHtml = newHtml.replace("SVGPATHD", exploreArray[i][1])

    exploreContainer.innerHTML += newHtml
}


let moreMidiasArray = [
    // [nome da opção, svg path]
    ["YouTube Premium", 'viewBox="0 0 97 20"', `
        <g>
...
htmlTemplate = `
    <div class="guide-option">
        <svg style="fill: currentcolor;" VIEWBOX enable-background="new 0 0 24 24" height="24" viewBox="0 0 24 24" width="24">
          <svg>
            SVGPATHSD
          </svg>
        </svg>
...
let midiasContainer = document.getElementById("Mais-Midias")

for (let i in moreMidiasArray) {
    let newHtml = htmlTemplate.replace("OPTIONNAME", moreMidiasArray[i][0])
    newHtml = newHtml.replace("VIEWBOX", moreMidiasArray[i][1])
    newHtml = newHtml.replace("SVGPATHSD", moreMidiasArray[i][2])

    midiasContainer.innerHTML += newHtml
}

let personalOptionsArray = [
    // [nome da opção, svg path]
    ["Configurações", "M12 9.5c1.38 0 2.5 1.12 2.5 2.5s-1.12 2.5-2.5 2.5-2.5-1.12-2.5-2.5 1.12-2.5 2.5-2.5m0-1c-1.93 0-3.5 1.57-3.5 3.5s1.57 3.5 3.5 3.5 3.5-1.57 3.5-3.5-1.57-3.5-3.5-3.5zM13.22 3l.55 2.2.13.51.5.18c.61.23 1.19.56 1.72.98l.4.32.5-.14 2.17-.62 1.22 2.11-1.63 1.59-.37.36.08.51c.05.32.08.64.08.98s-.03.66-.08.98l-.08.51.37.36 1.63 1.59-1.22 2.11-2.17-.62-.5-.14-.4.32c-.53.43-1.11.76-1.72.98l-.5.18-.13.51-.55 2.24h-2.44l-.55-2.2-.13-.51-.5-.18c-.6-.23-1.18-.56-1.72-.99l-.4-.32-.5.14-2.17.62-1.21-2.12 1.63-1.59.37-.36-.08-.51c-.05-.32-.08-.65-.08-.98s.03-.66.08-.98l.08-.51-.37-.36L3.6 8.56l1.22-2.11 2.17.62.5.14.4-.32c.53-.44 1.11-.77 1.72-.99l.5-.18.13-.51.54-2.21h2.44M14 2h-4l-.74 2.96c-.73.27-1.4.66-2 1.14l-2.92-.83-2 3.46 2.19 2.13c-.06.37-.09.75-.09 1.14s.03.77.09 1.14l-2.19 2.13 2 3.46 2.92-.83c.6.48 1.27.87 2 1.14L10 22h4l.74-2.96c.73-.27 1.4-.66 2-1.14l2.92.83 2-3.46-2.19-2.13c.06-.37.09-.75.09-1.14s-.03-.77-.09-1.14l2.19-2.13-2-3.46-2.92.83c-.6-.48-1.27-.87-2-1.14L14 2z"],
    ["Historico de denúncias", "m13.18 4 .24 1.2.16.8H19v7h-5.18l-.24-1.2-.16-.8H6V4h7.18M14 3H5v18h1v-9h6.6l.4 2h7V5h-5.6L14 3z"],
...
let personalOptionsContainer = document.getElementById("Personal-Options-Container")

for (let i in personalOptionsArray) {
    let newHtml = htmlTemplate.replace("OPTIONNAME", personalOptionsArray[i][0])
    newHtml = newHtml.replace("SVGPATHD", personalOptionsArray[i][1])
